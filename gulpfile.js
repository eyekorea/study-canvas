...
gulp.task('style',["sprity:normal"], function () {
	return style();
});
...
// sprite image 생성 ( 고 dpi 를 지원해야 하는 이미지 스프라이트.  )
// sprity
...
gulp.task("sprity:normal", function(){

	return sprity.src({
			src: 'src/assets/images/_sprite/*/*.png',
			split: true,
			template: 'src/assets/images/_sprite/template.hbs',
			prefix: 'spr',
			'style-indent-char': 'tab',
			cssPath: '/assets/images/common/',
			style: '_sprite.less',
			processor: 'less',
			name: 'sprite',
			margin: 0,
		...
		.pipe($.plumber())
		.pipe($.if('*.png', gulp.dest('build/assets/images/common/'), gulp.dest('src/assets/css/')))
		.pipe($.if(bs.active, bs.stream()));
});
...
gulp.task('watch', ['build'], function () {
	...
	// gulp.watch('src/assets/images/_sprite.normal/*/*.png', ['sprity:normal']);
